...
(function(){
"use strict";
const {VALS,QN,CARDS,TRIAGE,MODS,PROGRAMS,HOKUTO,EVIDENCE,BOOK,EPIS,DEF_EX,SELF_Q,LIKERT,SELF_T,WHY_WHAT,ASK5,ACT_EX,MINES,MINE_OPT,OT,REDFLAGS,CONDS,COND_OPT,HEX,RIASEC_Q,RI_OPT,DIRS,SOURCES,AX,AXP,B0,B0G,VERDICT}=window.Content;
...
function m2Check(){const list=m2List(),hl=effRank().filter(held),eps=EPIS.filter(e=>epText(e.k));
 ...
 ${guideHTML("体験が思い浮かばない価値は、「大事そうな言葉」を選んだだけかもしれないよ。そういう価値は「保留」にして、6位の価値と入れ替えよう","think")}
 ${list.map(card).join("")}
...
function m6Cond(n){const N=CONDS.length,C=condById[S.m6.order[n-1]],cur=S.m6.c[C.id],rel=condRel(C),mn=mustN();
 ...
 <div class="mtray" id="tray" role="group" aria-label="必須にした条件(3つまで)">${[0,1,2].map(k=>{const M=CONDS.filter(x=>S.m6.c[x.id]===3)[k];return M?`<button type="button" class="slotm" data-slot="${M.id}" aria-label="必須:${esc(M.n)}">◎ ${esc(M.n)}</button>`:`<span class="slotm empty" aria-hidden="true">◎</span>`;}).join("")}</div><div id="trEdit"></div>
 ...
 return{html:h,title:"理想の職場",bar:progBtn,after(){let busy=false;
  const tray=$("#tray"),ed=$("#trEdit");let swap=false;
  const next=v=>{busy=true;S.m6.c[C.id]=v;save();const b=$(`.tri button[data-v="${v}"]`);if(b)b.setAttribute("aria-pressed","true");const cc=$("#cc");if(!reduce)cc.classList.add(v===0?"gol":v===3?"gou":"gor");
   later(()=>go(n<N?"m6/cond/"+(n+1):"m6/ri/1"),260);};
  $$(".tri button").forEach(b=>b.onclick=()=>{if(busy)return;const v=+b.dataset.v;ed.innerHTML="";$$(".slotm.sel").forEach(x=>x.classList.remove("sel"));
   // 必須が3つ埋まっていたら、枠を揺らして入れ替え待ちにする
   if(v===3&&cur!==3&&mustN()>=3){swap=!swap;tray.classList.remove("swap");if(swap){void tray.offsetWidth;tray.classList.add("swap");}b.setAttribute("aria-pressed",String(swap));return;}
   swap=false;tray.classList.remove("swap");next(v);});
  $$("[data-slot]",tray).forEach(sb=>sb.onclick=()=>{if(busy)return;const id=sb.dataset.slot;
   if(swap){S.m6.c[id]=2;next(3);return;}
   // 枠のカードを選び直す
   const on=sb.classList.contains("sel");$$(".slotm.sel").forEach(x=>x.classList.remove("sel"));if(on){ed.innerHTML="";return;}sb.classList.add("sel");
   ed.innerHTML=`<div class="tredit">${[[2,"○","あると嬉しい"],[1,"−","どちらでも"],[0,"×","NG"]].map(([v,s2,l])=>`<button type="button" data-re="${v}"><span>${s2}</span> ${l}</button>`).join("")}</div>`;
   $$("[data-re]",ed).forEach(rb=>rb.onclick=()=>{S.m6.c[id]=+rb.dataset.re;save();keepScroll();});});
...
const scale=(attr,val,lo,hi,label)=>`<div class="scl"><div class="poles" aria-hidden="true"><span>${esc(lo)}</span><span>${esc(hi)}</span></div><div class="dots5" role="group" aria-label="${esc(label)}(左:${esc(lo)} 右:${esc(hi)})">${[1,2,3,4,5].map(v=>`<button type="button" ${attr}="${v}" aria-pressed="${val===v}" aria-label="${v}"><i style="--s:${8+v*4}px"></i></button>`).join("")}<button type="button" class="unk" ${attr}="u" aria-pressed="${val==null}" aria-label="分からない">?</button></div></div>`;
...
function scoreAxes(){const t=top5(),dw=[5,4,4,3,3];
 return t.map((c,i)=>({k:"v:"+c,l:c,w:dw[i],h:CAT[c]?VALS[VI[CAT[c]]].w:"",lo:"満たされなそう",hi:"よく満たされる"})).concat([{k:"c:rel",l:"人間関係・心理的安全性",w:3,h:"困ったときに相談できるか、意見を言えるか",lo:"相談しにくい",hi:"支え合える"},{k:"c:fair",l:"公正な評価と報酬",w:3,h:"評価基準の公開、努力が報われるか",lo:"あいまい・報われない",hi:"公平で報われる"},{k:"c:time",l:"労働時間・働き方",w:3,h:"残業時間、休日、転勤",lo:"合わない",hi:"希望どおり"}]);}
...
function m7Co(c){if(!isObj(c.m7.ngc))c.m7.ngc={};if(!isObj(c.m7.mustc))c.m7.mustc={};const ax=scoreAxes(),L=S.done.m6?m6Lists():{must:[],ng:[]},ngs=L.ng.map(C=>C.n).concat(S.done.m5?m5NG():[]);
 ...
 ${ax.map(x=>{const v=c.m7.s[x.k]!=null?+c.m7.s[x.k]:null;return `<div class="evcard axrow" data-k="${esc(x.k)}"><div class="evh"><b>${esc(x.l)}</b><span class="wt">重み×${x.w}</span></div>${x.h?`<p class="small" style="margin:0 0 6px">${esc(x.h)}</p>`:""}
  ${scale("data-sv",v,x.lo,x.hi,x.l)}
 ...
 return{html:h,title:"企業マッチ度",bar:progBtn,after(m){const tot=()=>{const r=coScore(c);$("#tot").textContent=`${r.s} / ${r.m}`;};tot();
  $$(".axrow",m).forEach(row=>{const k=row.dataset.k;bindSeg(row,"data-sv",v=>{if(v==="u")delete c.m7.s[k];else c.m7.s[k]=+v;save();tot();});});
...
const m8Of=c=>{if(!c.m8)c.m8={b0:[],mines:[null,null,null,null,null,null,null],ax:Array(12).fill(null),b3:{},b4:{},pre:"",friend:"",step:"b0",v:2};
 const B=c.m8;if(B.v!==2){B.ax=(Array.isArray(B.ax)?B.ax:[]).concat(Array(12)).slice(0,12).map(x=>x==null||+x===3?null:+x);B.v=2;}return B;};
...
function m8Verdict(c){const B=m8Of(c),t=top5(),imp=selfImp();
 ...
 const surf=B.b0.filter(k=>k[0]==="0"||k[0]==="1").length,daily=B.b0.filter(k=>k[0]==="2").length,unk=B.ax.filter(x=>x==null).length;
 let k,why;
 ...
 const notes=[];if(surf>daily)notes.push("惹かれる理由が、表面的な理由や選考体験にかたよっています(ハロー効果に注意)。");if(unk>=6)notes.push(`12の軸のうち${unk}軸が「分からない」。質問集で情報を取りに行こう。`);
...
function m8Step(c,st){const B=m8Of(c),i=M8STEPS.indexOf(st),t=top5(),imp=selfImp();let body="";
 ...
 if(st==="b2")body=`<h1 class="q">12の軸で、この会社を見てみよう</h1>
  ${AX.map((x,k)=>`<div class="evcard arow" data-i="${k}"><div class="evh"><b>${k+1} ${esc(x.n)}</b></div>${scale("data-av",B.ax[k],AXP[k][0],AXP[k][1],x.n)}<details class="qd"><summary>確かめる質問</summary><p class="small">${esc(x.q)}</p></details></div>`).join("")}`;
 ...
 if(st==="b4")body=`<h1 class="q">あなたと会社の「大事」は、ずれてない?</h1><p class="qhint">制度や社員の行動から見てみよう</p>
  ${VALS.map((v,vi)=>{const me=imp[v.n],co0=B.b4[v.n]!=null?+B.b4[v.n]:null,co=co0||3;return `<div class="evcard b4row" data-v="${esc(v.n)}"><details class="vd"><summary><b>${esc(v.n)}</b><span class="small">あなた ${"●".repeat(me)}${"○".repeat(5-me)}</span><span class="dif" style="color:${diffCol(co-me)}">${co-me>0?"+":""}${co-me}</span></summary><div class="vdin"><span class="cv" data-c="${vi}-${vi}x" data-w="54"></span><span>${esc(v.d)}<br><span class="small">仕事では:${esc(v.w)}</span></span></div></details>${scale("data-cv",co0,"ほとんど重視しない","とても重視している",v.n+"を会社がどれくらい重視しているか")}</div>`;}).join("")}`;
 ...
 return{html:h,title:"1社検証",bar:progBtn,after(m){
  ...
  $$(".arow",m).forEach(r=>bindSeg(r,"data-av",v=>{B.ax[+r.dataset.i]=v==="u"?null:+v;save();}));
  ...
  $$(".b4row",m).forEach(r=>{const v=r.dataset.v;bindSeg(r,"data-cv",x=>{if(x==="u")delete B.b4[v];else B.b4[v]=+x;save();const d=(x==="u"?3:+x)-imp[v],el=$(".dif",r);el.textContent=(d>0?"+":"")+d;el.style.color=diffCol(d);});});
 ...
 m8:{title:"1社検証の結果",file:"1社検証",
  md(){const c=coById(S.m8cur);if(!c||!c.verdict)return["(まだ結果がありません)"];const B=m8Of(c),V=m8Verdict(c),t=top5(),imp=selfImp();
   return[`## ${c.n}:${VERDICT[V.k].l}`,V.why+(V.notes.length?"\n\n"+V.notes.map(n=>`- ${n}`).join("\n"):""),
    ...
    "## B2 12の軸",mdTable(["軸","点数(1〜5)"],AX.map((x,k)=>[`${k+1} ${x.n}`,B.ax[k]==null?"分からない":B.ax[k]])),
  ...
  html(){const c=coById(S.m8cur);if(!c||!c.verdict)return `<p>(まだ結果がありません)</p>`;const B=m8Of(c),V=m8Verdict(c),t=top5(),imp=selfImp();
   ...
   <h3>B2 12の軸</h3><table>${AX.map((x,k)=>`<tr><td>${k+1} ${esc(x.n)}</td><td><b>${B.ax[k]==null?"?":B.ax[k]}</b></td></tr>`).join("")}</table>
